...
// --- (useIsMounted, useWindowWidth 훅 - 변경 없음) ---
const useIsMounted = () => {
...
const WorksPage = () => {
  ...
  const dataKeyToCategory: { [key in keyof typeof WORKS]?: CategoryType } = useMemo(() => ({
    COMMUNICATION: 'Communication Design',
    SERVICE: 'Service Design',
    UX: 'UX Design',
    INDUSTRIAL: 'Industrial Design',
  ...
  useEffect(() => {
    ...
    // selectedCategory를 의존성 배열에서 제거하여 불필요한 재실행을 방지 (useEffect 내에서 업데이트하기 때문)
  ...
  const projects: Project[] = useMemo(() => { 
    ...
    (Object.keys(WORKS) as (keyof typeof WORKS)[]).forEach(dataKey => {
      ...
      worksArray.forEach(work => {
        ...
        allProjects.push({
          ...
          category: categoryUI,
          // 경고를 해결하기 위해 Image 컴포넌트 사용을 가정하고 src를 설정
    ...
    return allProjects;
  }, [dataKeyToCategory]); // dataKeyToCategory를 useMemo로 감쌌기 때문에 의존성 배열에 추가
  ...
  return (
    <div className="min-h-screen">
     ...
     <div className="lg:hidden sticky top-0 z-50 bg-white">
        ...
        {/* --- 모바일 필터 (Overflow-x-auto로 가로 스크롤 가능) --- */}
        <nav className="flex gap-2 p-4 overflow-x-auto lg:hidden bg-white border-b border-gray-200">
           {categories.map((category, index) => (
            <React.Fragment key={category}>
              ...
              >
                {/* 데스크톱 아이콘 (hidden lg:inline-block) */}
                {category !== 'All' && (
                  <span
                    className={cn(
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        {/* --- 메인 프로젝트 그리드 (수정됨: lg:ml-1/4 추가) --- */}
        <main className="flex-1 p-4 lg:p-8 lg:ml-1/4">
              ...
              {filteredProjects.map((project) => (
                ...
                >
                  <div className="aspect-[3/2] lg:w-[336px] lg:h-[254px] mb-2 relative transition-all lg:group-hover:outline lg:group-hover:outline-2 lg:group-hover:outline-[#00FF36] lg:group-hover:outline-offset-[-2px]">
                    ...
                    {/* Next/Image 컴포넌트로 변경하여 성능 경고 해결 권장 */}
                    <img
                      src={project.image}
                      alt={project.name}
                      // placeholder="blur" // 실제 Next/Image 사용 시 추가
                      // blurDataURL="..." // 실제 Next/Image 사용 시 추가
                      // layout="fill" // 실제 Next/Image 사용 시 추가
                      // objectFit="cover" // 실제 Next/Image 사용 시 추가
      ...
      {/* --- 푸터 (가장 작은 화면 너비(390px) 이하일 때 모바일 푸터, 그 외는 일반 푸터) --- */}

...
// [수정됨] 1. 모바일 푸터 컴포넌트를 import 합니다.
// (파일 경로는 실제 위치에 맞게 조정해야 합니다.)
...
// [수정됨] 2. 현재 창 너비를 감지하는 커스텀 훅 추가
const useWindowWidth = () => {
  const [width, setWidth] = useState(0);

  useEffect(() => {
    // window 객체가 존재할 때만 실행 (서버 사이드 렌더링 오류 방지)
    if (typeof window !== 'undefined') {
      const handleResize = () => {
        setWidth(window.innerWidth);
      };

      // 컴포넌트 마운트 시 초기 너비 설정 및 리사이즈 이벤트 리스너 추가
      handleResize();
  ...
  }, []); // 빈 배열: 이 effect는 클라이언트에서 한 번만 실행됩니다.
